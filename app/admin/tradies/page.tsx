...
function DocumentsTab() {
  ...
  const fetchTradies = useCallback(async () => {
    setIsLoading(true)
    try {
      const res = await api.getPaginated<PendingTradie>(
        `/api/admin/tradies/pending?page=${page}&limit=20`
      )
      setTradies(res.data)
      setTotal(res.pagination.total)
    } catch {
      // The shared API client surfaces request failures.
    } finally {
      setIsLoading(false)
    }
  }, [page])
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="min-w-0">
                  <h3 className="text-sm font-semibold text-gray-800 truncate group-hover:text-[#2563EB] transition-colors">
                    {user.name}
                  </h3>
              ...
              <div className="flex flex-wrap gap-1 mb-4">
                {tradie.categories.slice(0, 3).map((cat) => (
                  <span
                    key={cat}
                    className="text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-600"
                  >
...
export default function AdminTradiesPage() {
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Tradie Verification</h1>
        <p className="text-sm text-gray-400 mt-0.5">
          Review and verify tradie compliance documents
        </p>
      </div>
