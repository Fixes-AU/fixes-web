...
const initial: State = { analytics: null, audit: [], period: '30', loading: true, exporting: null, stale: false, error: null }
function reducer(state: State, action: Action): State {
  switch (action.type) {
    case 'LOAD': return { ...state, loading: !state.analytics, error: null }
    case 'SUCCESS': return { ...state, analytics: action.analytics, audit: action.audit, loading: false, stale: false, error: null }
    case 'ERROR': return { ...state, loading: false, stale: Boolean(state.analytics), error: action.error }
...
export default function MarketingInsights({ campaignId, campaignName, canExport }: { campaignId?: string; campaignName?: string; canExport: boolean }) {
