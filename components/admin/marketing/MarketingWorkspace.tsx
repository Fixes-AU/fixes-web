...
import {
  ...
  CirclePlay, Edit3, HelpCircle, Loader2, Megaphone, MoreHorizontal, Plus, RefreshCw,
  Search, ShieldAlert, Ticket, Users, X, XCircle,
} from 'lucide-react'
...
import MarketingRegistrations from './MarketingRegistrations'
...
export default function MarketingWorkspace() {
  ...
  const [activeView, setActiveView] = useState<'campaigns' | 'registrations' | 'analytics' | 'operations'>('campaigns')
  const [refreshToken, setRefreshToken] = useState(0)
  const [pendingSectionRefreshes, setPendingSectionRefreshes] = useState(0)
  const [mainRefreshDone, setMainRefreshDone] = useState(true)
  ...
  const sectionRefreshComplete = useCallback((token: number) => {
    if (refreshTokenRef.current !== token) return
    setPendingSectionRefreshes(current => Math.max(0, current - 1))
  }, [])
  ...
  const refreshAll = useCallback(async () => {
    ...
    setPendingSectionRefreshes(3)
    setMainRefreshDone(false)
    setRefreshingAll(true)
    setBulkRefreshKey(value => value + 1)
    const requests: Promise<unknown>[] = [actions.loadCampaigns(true)]
    if (state.selectedCampaign) requests.push(actions.selectCampaign(state.selectedCampaign._id))
    await Promise.allSettled(requests)
    setMainRefreshDone(true)
  }, [actions, refreshingAll, state.selectedCampaign])

  useEffect(() => {
    if (refreshingAll && mainRefreshDone && pendingSectionRefreshes === 0) setRefreshingAll(false)
  }, [mainRefreshDone, pendingSectionRefreshes, refreshingAll])
  ...
  return <div className="max-w-7xl mx-auto space-y-6">
    <header className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
      ...
      <div className="flex gap-2">
        <button disabled={refreshingAll} onClick={() => void refreshAll()} className="marketing-button-secondary disabled:opacity-60"><RefreshCw className={`w-4 h-4 ${refreshingAll ? 'animate-spin' : state.stale ? 'text-amber-500' : ''}`} /> {refreshingAll ? 'Refreshing' : 'Refresh all'}</button>
    ...
    <nav className="grid grid-cols-2 gap-2 rounded-2xl border border-gray-200 bg-white p-2 lg:grid-cols-4" aria-label="Marketing workspace sections">
      <WorkspaceTab active={activeView === 'campaigns'} icon={Megaphone} label="Campaigns" help="Create campaigns, distribution links and discount codes." onClick={() => setActiveView('campaigns')} />
      <WorkspaceTab active={activeView === 'registrations'} icon={Users} label="Registrations" help="Inspect each attributed Fixes, Fixer, cleaning, agency and waitlist signup." onClick={() => setActiveView('registrations')} />
      <WorkspaceTab active={activeView === 'analytics'} icon={BarChart3} label="Analytics & audit" help="Review funnel conversion, campaign activity and immutable staff actions." onClick={() => setActiveView('analytics')} />
      <WorkspaceTab active={activeView === 'operations'} icon={ShieldAlert} label="Operations" help="Check checkout readiness, incidents and rollout safety switches." onClick={() => setActiveView('operations')} />
    </nav>

    <div className={activeView === 'campaigns' ? 'space-y-6' : 'hidden'}>
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      <Metric icon={Megaphone} label="Campaigns" help="Total campaigns matching the current search and status filter." value={state.total} />
      <Metric icon={CirclePlay} label="Active shown" help="Active campaigns on the currently loaded page." value={state.campaigns.filter(item => item.status === 'active').length} accent="emerald" />
      <Metric icon={Ticket} label="Selected codes" help="Codes belonging to the open campaign. Open a campaign to populate this value." value={state.codes.length} accent="violet" />
      <Metric icon={BarChart3} label="Committed budget" help="Reserved, authorized and redeemed subsidy value for the currently loaded campaigns." value={formatAud(state.campaigns.reduce((sum, item) => sum + item.reservedBudgetCents + item.authorizedBudgetCents + item.redeemedBudgetCents, 0))} accent="amber" />
    </div>
    ...
    {state.detailLoading ? <div className="rounded-2xl border border-gray-200 bg-white py-20 flex justify-center"><Loader2 className="w-6 h-6 animate-spin text-violet-600" /></div> : state.selectedCampaign && <CampaignDetail campaign={state.selectedCampaign} codes={state.codes} canManage={Boolean(state.access?.canManage)} canExport={Boolean(state.access?.canExport)} busy={state.mutating} bulkRefreshKey={bulkRefreshKey} onClose={actions.clearSelection} onEdit={() => setCampaignForm({ open: true, campaign: state.selectedCampaign })} onCreateCode={() => setCodeForm({ open: true, code: null })} onEditCode={code => setCodeForm({ open: true, code })} onBulkCode={setBulkSource} onDefaultSignupOffer={saveDefaultSignupOffer} onCampaignTransition={nextStatus => setTransition({ kind: 'campaign', campaign: state.selectedCampaign!, nextStatus })} onCodeTransition={(code, nextStatus) => setTransition({ kind: 'code', campaign: state.selectedCampaign!, code, nextStatus })} />}
    </div>
    <div className={activeView === 'registrations' ? 'block' : 'hidden'}><MarketingRegistrations campaignId={state.selectedCampaign?._id} campaignName={state.selectedCampaign?.publicName} canExport={Boolean(state.access?.canExport)} refreshToken={refreshToken} onRefreshComplete={sectionRefreshComplete} /></div>
    <div className={activeView === 'analytics' ? 'block' : 'hidden'}><MarketingInsights campaignId={state.selectedCampaign?._id} campaignName={state.selectedCampaign?.publicName} canExport={Boolean(state.access?.canExport)} refreshToken={refreshToken} onRefreshComplete={sectionRefreshComplete} /></div>
    <div className={activeView === 'operations' ? 'block' : 'hidden'}><MarketingOperations refreshToken={refreshToken} onRefreshComplete={sectionRefreshComplete} /></div>
...
function CampaignDetail({ campaign, codes, canManage, canExport, busy, bulkRefreshKey, onClose, onEdit, onCreateCode, onEditCode, onBulkCode, onDefaultSignupOffer, onCampaignTransition, onCodeTransition }: { campaign: MarketingCampaign; codes: DiscountCode[]; canManage: boolean; canExport: boolean; busy: boolean; bulkRefreshKey: number; onClose: () => void; onEdit: () => void; onCreateCode: () => void; onEditCode: (code: DiscountCode) => void; onBulkCode: (code: DiscountCode) => void; onDefaultSignupOffer: (codeId: string | null, token: string) => Promise<void>; onCampaignTransition: (status: CampaignStatus) => void; onCodeTransition: (code: DiscountCode, status: DiscountCodeStatus) => void }) {
  return <section className="rounded-2xl border border-gray-200 bg-white overflow-hidden"><div className="p-5 border-b border-gray-100 flex flex-col lg:flex-row lg:items-start justify-between gap-4"><div><div className="flex items-center gap-2"><h2 className="text-lg font-bold text-gray-900">{campaign.publicName}</h2><StatusBadge status={campaign.status} /></div><p className="text-sm text-gray-500 mt-1">{campaign.termsSummary}</p><p className="text-[10px] text-gray-400 mt-2">Version {campaign.version} · Updated {new Date(campaign.updatedAt).toLocaleString('en-AU')}</p></div><div className="flex items-center gap-2">{canManage && <DropdownMenu><DropdownMenuTrigger asChild><button type="button" className="marketing-button-secondary"><MoreHorizontal className="h-4 w-4" /> Actions</button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-52"><DropdownMenuItem onSelect={onEdit}><Edit3 /> Edit campaign</DropdownMenuItem>{(campaignTransitions[campaign.status] || []).map(status => <DropdownMenuItem key={status} variant={['ended', 'archived'].includes(status) ? 'destructive' : 'default'} onSelect={() => onCampaignTransition(status)}>{status === 'active' ? <CirclePlay /> : status === 'paused' ? <CirclePause /> : <Archive />} Move to {CAMPAIGN_STATUS_LABELS[status]}</DropdownMenuItem>)}</DropdownMenuContent></DropdownMenu>}<Tooltip><TooltipTrigger asChild><button type="button" onClick={onClose} className="marketing-button-secondary" aria-label="Close campaign details"><X className="h-4 w-4" /></button></TooltipTrigger><TooltipContent>Close this campaign card</TooltipContent></Tooltip></div></div>
...
function Metric({ icon: Icon, label, help, value, accent = 'blue' }: { icon: React.ElementType; label: string; help: string; value: string | number; accent?: string }) { const colors: Record<string, string> = { blue: 'bg-blue-50 text-blue-600', emerald: 'bg-emerald-50 text-emerald-600', violet: 'bg-violet-50 text-violet-600', amber: 'bg-amber-50 text-amber-600' }; return <div className="rounded-xl border border-gray-200 bg-white p-4 flex gap-3"><div className={`rounded-lg p-2 h-fit ${colors[accent]}`}><Icon className="w-4 h-4" /></div><div><p className="flex items-center gap-1 text-xs text-gray-400">{label}<Tooltip><TooltipTrigger asChild><span role="img" aria-label={`About ${label}`}><HelpCircle className="h-3 w-3" /></span></TooltipTrigger><TooltipContent className="max-w-xs">{help}</TooltipContent></Tooltip></p><p className="text-xl font-bold text-gray-900">{value}</p></div></div> }
function WorkspaceTab({ active, icon: Icon, label, help, onClick }: { active: boolean; icon: React.ElementType; label: string; help: string; onClick: () => void }) { return <button type="button" onClick={onClick} className={`flex items-center justify-between gap-2 rounded-xl px-3 py-3 text-left text-sm font-semibold transition ${active ? 'bg-violet-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-50'}`}><span className="flex items-center gap-2"><Icon className="h-4 w-4" />{label}</span><Tooltip><TooltipTrigger asChild><span role="img" aria-label={`About ${label}`} onClick={event => event.stopPropagation()} className={active ? 'text-violet-200' : 'text-gray-400'}><HelpCircle className="h-3.5 w-3.5" /></span></TooltipTrigger><TooltipContent className="max-w-xs">{help}</TooltipContent></Tooltip></button> }
