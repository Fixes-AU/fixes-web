...
export interface Job {
  ...
  images: JobImage[]
  completionPhotos: JobImage[]   // watermarked work-proof photos submitted by tradie at completion
  location: JobLocation
  preferredTime: PreferredTime
  scheduledFor: string | null
  status: JobStatus
  selectedTier: SkillLevel | null  // which quote tier the client selected
  isAfterHours: boolean            // true if job was posted after 6pm AU local time
...
// One tier option embedded inside the Quote document
...
export interface Quote {
  ...
  options: QuoteOption[]          // 1-4 tier options generated by AI
  selectedTier: SkillLevel | null // Set when client picks a tier
  engine: QuoteEngine
...
// Response returned by PATCH /api/jobs/:id/cancel
export interface CancelJobResponse {
  job: Job
  cancellationFeeAmount: number   // Total AUD the client does NOT get back
  tradiePayout: number      // Portion sent to tradie as compensation
  platformRevenue: number   // Portion kept by platform
  refundAmount: number      // Amount being refunded to card
  currency: 'AUD'
}
...
export interface DispatchNewPayload {
  ...
  distance: number
  expiresAt: string
  // Cancellation response fields (returned when client cancels with fee)
